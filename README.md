...
（6 个账号 + 4 个活动 + 6 条覆盖四种状态的报名记录，详见第六节）。
...
- [x] 阶段 7 收尾：实验二报告已填写并签名、tag `v2.0` 已打
- [x] 提交包已打包（`校园活动管理系统_V2.0_提交包.zip`，78 个文件 / 2.86 MB）

### 重新打包提交包

```powershell
python docs\tools\pack_submission.py
```

包内含源码、两个依赖 jar、`build.bat`、README、实验二报告（已签名）与 docs 下的文档；
**不含** `frontend/node_modules`、`frontend/dist`、课程材料与开发期脚本。
完整清单见 [`docs/作业2/V2.0开发任务清单.md`](docs/作业2/V2.0开发任务清单.md) 的 7.5 节。

> ⚠️ 报告已签名。**不要**直接重跑 `fill_report2.py` —— 它从空白原版重新生成，会冲掉签名。
> 脚本已加保护：检测到签名会拒绝覆盖（确需重新生成才加 `--force`，之后要重新签名）。
